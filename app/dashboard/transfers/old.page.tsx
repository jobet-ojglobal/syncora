...
export default function TransferOrdersListPage() {
  ...
  const [receivedLinesState, setReceivedLinesState] = useState<
    Record<string, { quantityReceived: number; targetSublocationId?: string, discrepancyReason: string }>
  >({});
  ...
  const openStatusActionModal = (
    ...
    const initialLinesState: Record<string, { quantityReceived: number; targetSublocationId?: string, discrepancyReason?: string }> = {};
    ...
    <div className="w-full max-w-6xl mx-auto p-6 space-y-6">
      ...
      <Dialog open={!!activeAction} onOpenChange={(open) => !open && setActiveAction(null)}>
        <DialogContent className="max-w-3xl">
          ...
          {activeAction && (
            <div className="space-y-4 my-2 text-xs">
              {/* Receiving Discrepancy Form Grid */}
                ...
                <div className="space-y-3 border rounded-lg p-3 bg-slate-50/50 dark:bg-slate-900/50">
                  <div className="flex items-center justify-between">
                    <Label className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">
                      Verify Incoming Line Quantities & Categorize Discrepancies
                    </Label>
                    <span className="text-[10px] text-muted-foreground">
                      Log missing or damaged units with structured discrepancy tags.
                    </span>
                  </div>

                  <div className="border rounded-md bg-background overflow-hidden max-h-72 overflow-y-auto">
                    <table className="w-full text-left text-xs">
                      <thead className="bg-muted/40 border-b text-[10px] uppercase font-semibold sticky top-0 bg-background">
                        <tr>
                      ...
                      <tbody className="divide-y">
                        {activeAction.order.lines.map((line) => {
                          const lineState = receivedLinesState[line.id] || {
                            quantityReceived: line.quantity,
                            discrepancyReason: "",
                          };
                          const currentReceived = lineState.quantityReceived;
                          const diff = currentReceived - line.quantity;
                          const hasDiscrepancy = diff !== 0;

                          return (
                            <tr key={line.id} className={hasDiscrepancy ? "bg-amber-500/5" : ""}>
                              ...
                              <td className="p-2">
                                {hasDiscrepancy ? (
                                  <Select
                                    value={lineState.discrepancyReason || ""}
                                    onValueChange={(val) => {
                                      setReceivedLinesState((prev) => ({
                                        ...prev,
                                        [line.id]: {
                                  ...
                                  >
                                    ...
                                    <SelectContent>
                                      ...
                                      <SelectItem value="MISSING_BOX">Missing Box / Shrinkage</SelectItem>
                                      <SelectItem value="VENDOR_SHORTAGE">Vendor / Dispatch Shortage</SelectItem>
                                      <SelectItem value="OVERAGE_UNCOUNTED">Overage / Extra Shipped</SelectItem>
                                      <SelectItem value="OTHER">Other Variance</SelectItem>
                                    </SelectContent>
                                  </Select>
                                ) : (
                                  <span className="text-[10px] text-muted-foreground italic">N/A (Match)</span>
                                )}
                              </td>
                            </tr>
                          );
              ...
              <div className="space-y-1.5">
                <Label htmlFor="action-remarks" className="text-xs font-semibold">
                  Update Manifest Remarks / Inspection Findings (Optional)
                </Label>
                <Textarea
                  id="action-remarks"
                  placeholder="Add specific details regarding damage claims, carrier notes, or inventory discrepancies..."
                  value={actionRemarks}
