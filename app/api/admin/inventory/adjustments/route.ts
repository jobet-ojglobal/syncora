  ...
  try {
    ...
    const where: Prisma.InventoryAdjustmentWhereInput = {
      AND: [
        search
          ...
          : {},
        status ? { status: status as Prisma.EnumAdjustmentStatusFilter } : {},
        reason
          ? {
              OR: [
                { adjustmentReason: { name: { contains: reason, mode: "insensitive" } } },
                { lines: { some: { reason: reason as any } } },
              ],
            }
          : {},
      ],
    ...
    const formattedData = rawAdjustments.map((adj) => {
      const warehouseNames = Array.from(
        new Set(adj.lines.map((l) => l.location?.name).filter(Boolean))
      );
      ...
      const netQuantityDelta = adj.lines.reduce(
        (sum, line) => sum + Number(line.quantityAdjusted || 0),
        0
      );

      // Extract reason from header relation or fallback to first line's reason
      const rawReason = adj.adjustmentReason?.name || adj.lines[0]?.reason;

      return {
        id: adj.id,
        referenceNo: adj.adjustmentNumber,
        reason: formatReasonLabel(rawReason),
        status: formatStatusLabel(adj.status),
        rawStatus: adj.status,
        adjustedBy: {
...
//         });
//       }
>>>>>>> dd9bab295ca918cde8394134d9371c7d168b5954
