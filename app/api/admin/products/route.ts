    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.product.update({
        ...
        data: {
          ...
          hsTariffNumber: hsTariffNumber?.trim() || null,
          remarks: remarks?.trim() || null,
          standardUomName: standardUomName && standardUomName.trim(),
          customFields: toJsonInput(customFields)
        }
      });
      ...
      if (purchasingUomId) {
        ...
        await tx.purchasingUom.deleteMany({ where: { productId: inflowId } });
      }

      // H. Sales UOM Sync
      if (salesUomId) {
        await tx.salesUom.upsert({
          where: { productId: inflowId },
          update: {
            uomId: salesUomId,
            standardQuantity: Number(salesUom.standardQuantity) || 1,
        ...
        await tx.salesUom.deleteMany({ where: { productId: inflowId } });
      }
