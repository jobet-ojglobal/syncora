...
import { generateSkuSlug } from "@/helpers/genSKU";
...
export function ProductForm({ brands, initialData }: ProductFormProps) {
  ...
  // Auto-generate Slug helper from product names
  const watchedName = useWatch({ control, name: "name" });
  const generateSlugFromName = () => {
    if (!watchedName) return;
    const computedSlug = watchedName
      .toLowerCase()
      .replace(/[^a-z0-9\s-]/g, "")
      .replace(/\s+/g, "-")
      .replace(/-+/g, "-");
    setValue("slug", computedSlug);
  };

  const generateSkuSlugFromName = () => {
     if (!watchedName) return
    const sku = generateSkuSlug(watchedName)
    setValue("sku", sku);
  }
        ...
        <FieldSet className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Field className="md:col-span-2">
            ...
            <div className="flex gap-2">
              ...
              {!isEditMode && <Button type="button" variant="outline" size="sm" onClick={generateSlugFromName} className="text-xs h-9 shrink-0">Slugify</Button>}
            </div>
          ...
          <Field className="md:col-span-2">
            <FieldLabel>Unique SEO Slug Route *</FieldLabel>
            <Input placeholder="premium-ergonomic-office-chair" {...register("slug")} />
