...
import { LocationItem } from "@/context/LocationWorkspaceContext"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function LocationSwitcher({ locations, onLocationChange, currentLocationId }: LocationSwitcherProps) {
      ...
      <SidebarMenuItem>
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-(--radix-dropdown-menu-trigger-width) min-w-56 rounded-lg" align="start" side={isMobile ? "bottom" : "right"}>
            <DropdownMenuLabel className="text-xs text-muted-foreground">Locations Workspace</DropdownMenuLabel>
            {locations.map((loc, index) => (
              <DropdownMenuItem key={loc.id} onClick={() => onLocationChange(loc.id)} className="gap-2 p-2 flex items-center justify-between">
    ...
    <Dialog open={isAddBranchOpen} onOpenChange={setIsAddBranchOpen}>
      ...
      <DialogContent className="sm:max-w-7xl">
        <DialogHeader>
          ...
          <DialogDescription className="text-sm text-muted-foreground">Configure a new operational workspace, warehouse, or fulfillment center location.</DialogDescription>
        </DialogHeader>
        {/* Location Form */}
        <LocationForm onSuccess={() => setIsAddBranchOpen(false)}/>
      </DialogContent>
    </Dialog>
  </>
  );
