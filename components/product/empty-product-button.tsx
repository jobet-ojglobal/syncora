// frontend/components/EmptyProductsButton.tsx
"use client";
...
interface EmptyProductsButtonProps {
  onSuccess?: () => void;
}

export function EmptyProductsButton({ onSuccess }: EmptyProductsButtonProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [progressMessage, setProgressMessage] = useState("");

  const handleEmptyProducts = async () => {
    setIsLoading(true);
    setError(null);
    setProgressMessage("Starting cleanup...");

    try {
      const response = await fetch(`/api/admin/products/empty`, {
        method: "POST",
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          if (payload.type === "progress") {
            setProgressMessage(
              `Deleted batch of ${payload.batchCount} products (Total: ${payload.totalDeleted})`
            );
            ...
            <div className="p-6">
              <div className="flex items-center gap-3 text-red-600 mb-4">
                ...
                <h3 className="text-lg font-semibold text-gray-900">
                  Empty Products Catalog
                </h3>
              </div>

              <p className="text-sm text-gray-600 leading-relaxed mb-4">
                Are you sure you want to delete <span className="font-semibold text-gray-900">ALL products</span> and their associated records (vendor items, pricing, location maps, BOM references)?
