// frontend/components/EmptyBPartnersButton.tsx
"use client";
...
interface EmptyBPartnersButtonProps {
  role: "ALL" | "CUSTOMER" | "VENDOR";
  onSuccess?: () => void;
}

export function EmptyBPartnersButton({
  role,
  onSuccess,
}: EmptyBPartnersButtonProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [progressMessage, setProgressMessage] = useState("");

  const handleEmptyBPartner = async () => {
    setIsLoading(true);
    setError(null);
    setProgressMessage("Starting cleanup...");

    try {
      const response = await fetch(`/api/admin/business-partners/empty`, {
        method: "POST",
