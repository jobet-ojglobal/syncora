...
export async function syncProduct(
  ...
  if (brandCustomName) {
    ...
    const rawBrandName = product.customFields?.[customKey];
  ...
  // if (product.name) {
  //   const shortId = product.productId.split("-")[0];
  //   const safeName = product.name
  //     .toLowerCase()
  //     .replace(/[^a-z0-9]/g, "-")
  //     .replace(/-+/g, "-");
  //   const link = `https://res.cloudinary.com/brd97n5v/image/upload/v1787643737/products/%${product.productId}%-${safeName}.jpg`;
  //   const accessible = await isImageAccessible(link);
  ...
  //     ];
  //   } else {
  //     console.log("Image missing/unavailable");
  //   }
  // }
  ...
  if (hasCoreProductData) {
    ...
    validProductData = localProduct as Product | null;
  }

  if (!validProductData) return null;

  // 5. UOM & Image Syncing Helpers
  if (product.purchasingUom?.name) await syncPurchasingUom(tx, product);
  if (product.salesUom?.name) await syncSalesUom(tx, product);
  if (product.images !== undefined) await syncImages(tx, product);
  ...
  //       const found = await tx.taxingScheme.findMany({
  //         where: { inflowId: { in: missingSchemeIds } },
  //         select: { inflowId: true },
  //       });
  //       found.forEach((s) => verifiedTaxingSchemes.add(s.inflowId));
  //     }
  ...
  //       const found = await tx.taxCode.findMany({
  //         where: { inflowId: { in: missingCodeIds } },
  //         select: { inflowId: true },
  //       });
  //       found.forEach((c) => verifiedTaxCodes.add(c.inflowId));
  //     }
  ...
  //     const neededLocations = product.reorderSettings
  //       .flatMap((rs) => [rs.locationId, rs.fromLocationId])
  //       .filter((id): id is string => Boolean(id) && !verifiedLocations.has(id));

  //     if (neededLocations.length) {
  //       const foundLocs = await tx.location.findMany({
  //         where: { inflowId: { in: neededLocations } },
  //         select: { inflowId: true },
  //       });
  //       foundLocs.forEach((l) => verifiedLocations.add(l.inflowId));
  //     }

  //     const neededVendors = product.reorderSettings
  //       .map((rs) => rs.vendorId)
  //       .filter((id): id is string => Boolean(id) && !verifiedVendors.has(id));

  //     if (neededVendors.length) {
  //       const foundVendors = await tx.vendor.findMany({
  //         where: { inflowId: { in: neededVendors } },
  //         select: { inflowId: true },
  //       });
  //       foundVendors.forEach((v) => verifiedVendors.add(v.inflowId));
  //     }

  //     const validReorderSettingsToCreate = product.reorderSettings
  //       .filter((rs) => rs.reorderSettingsId && rs.locationId && verifiedLocations.has(rs.locationId))
  //       .map((rs) => ({
  //         inflowId: rs.reorderSettingsId!,
  //         productId: product.productId,
  //         locationId: rs.locationId!,
  //         fromLocationId: rs.fromLocationId && verifiedLocations.has(rs.fromLocationId) ? rs.fromLocationId : null,
  //         vendorId: rs.vendorId && verifiedVendors.has(rs.vendorId) ? rs.vendorId : null,
  ...
  //     const neededOperationTypes = product.productOperations
  //       .map((po) => po.operationTypeId)
  //       .filter((id): id is string => Boolean(id) && !verifiedOperationTypes.has(id));

  //     if (neededOperationTypes.length) {
  //       const found = await tx.operationType.findMany({
  //         where: { inflowId: { in: neededOperationTypes } },
  //         select: { inflowId: true },
  //       });
  //       found.forEach((o) => verifiedOperationTypes.add(o.inflowId));
  //     }

  //     const validOperationsToCreate = product.productOperations
  //       .filter((po) => po.productOperationId && po.operationTypeId && verifiedOperationTypes.has(po.operationTypeId))
  //       .map((po) => ({
  ...
  if (product.prices !== undefined) {
    ...
    if (product.prices?.length) {
      const neededSchemes = product.prices
        .map((p) => p.pricingSchemeId)
        .filter((id): id is string => Boolean(id) && !verifiedPricingSchemeIds.has(id));

      if (neededSchemes.length) {
        const found = await tx.pricingScheme.findMany({
          where: { inflowId: { in: neededSchemes } },
          select: { inflowId: true },
        });
        found.forEach((s) => verifiedPricingSchemeIds.add(s.inflowId));
      }

      const validPricesToCreate = product.prices
        .filter((p) => p.productPriceId && p.pricingSchemeId && verifiedPricingSchemeIds.has(p.pricingSchemeId))
        .map((p) => ({
          inflowId: p.productPriceId!,
          pricingSchemeId: p.pricingSchemeId!,
          productId: product.productId,
          priceType: (p.priceType?.toLowerCase().includes("markup") ? "FixedMarkup" : "FixedPrice") as ProductPriceType,
  ...
  // 11. Product Cost Sync (1:1 Relation)
