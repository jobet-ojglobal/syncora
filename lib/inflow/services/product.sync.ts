...
export async function syncProduct(
  ...
  if (product.inventoryLines !== undefined) {
    const inventoryLines = product.inventoryLines ?? [];

    // =========================================================================
    // 1. LOCATION & SUBLOCATION BATCH SYNC
    // =========================================================================

    // Collect unique locations to sync
    ...
    // A. Sync missing Locations using shared utility
    for (const [locId, locData] of uniqueLocationsToSync) {
      await syncLocation(tx, locData);
      verifiedLocations.add(locId);
    }

    // B. Collect and deduplicate all required Sublocations (including "Default")
    const requiredSublocationKeys = new Set<string>(); // Format: "locationId_sublocationName"
    const sublocationsToCreate: { locationId: string; name: string }[] = [];

    for (const line of inventoryLines) {
      if (!line.locationId) continue;

      // Always guarantee "Default" sublocation existence per location
      const defaultKey = `${line.locationId}_Default`;
      if (!requiredSublocationKeys.has(defaultKey)) {
        requiredSublocationKeys.add(defaultKey);
        sublocationsToCreate.push({ locationId: line.locationId, name: "Default" });
      }

      // Capture dynamic sublocation from inventory line
      const subName = line.sublocation?.trim() || "Default";
      const customKey = `${line.locationId}_${subName}`;
      if (!requiredSublocationKeys.has(customKey)) {
        requiredSublocationKeys.add(customKey);
        sublocationsToCreate.push({ locationId: line.locationId, name: subName });
      }
    }

    // Bulk upsert all missing sublocations in parallel
    ...
    // Cache resolved sublocation IDs into memory map: "locationId_sublocationName" -> Sublocation.id (cuid)
    const targetLocationIds = [...new Set(inventoryLines.map((l) => l.locationId))];
    ...
    // =========================================================================
    // 2. AGGREGATE LOCATION TOTALS & BATCH INVENTORY UPSETS
    // =========================================================================
    ...
    for (const line of inventoryLines) {
      ...
      locationTotals.set(line.locationId, current.plus(lineQty));
    }

    // Fetch current inventory records across target locations for ledger comparison
    ...
    // =========================================================================
    // 3. WRITE INVENTORY, BINS & LEDGER ENTRIES
    // =========================================================================
    ...
    for (const [locationId, newTotalQty] of locationTotals) {
      ...
      // Upsert main Inventory record using composite key [productId, locationId]
      const inventory = await tx.inventory.upsert({
        where: {
          productId_locationId: { productId: product.productId, locationId },
        },
        create: {
          productId: product.productId,
          locationId,
      ...
      // Collect resolved sublocation UUIDs for bins in this location
      const targetSublocationIds = linesForLocation
        .map((line) => {
          const subName = line.sublocation?.trim() || "Default";
          return sublocationIdMap.get(`${locationId}_${subName}`);
        })
        .filter((id): id is string => Boolean(id));

      // Batch pre-fetch existing bins to compute delta changes without extra database queries
      const existingBins = await tx.inventoryBin.findMany({
        where: {
          inventoryId: inventory.id,
          sublocationId: { in: targetSublocationIds },
        },
      });
      ...
      // Process Bin updates & Ledger entries concurrently
      await Promise.all(
        linesForLocation.map(async (line) => {
          const subName = line.sublocation?.trim() || "Default";
          const sublocationId = sublocationIdMap.get(`${locationId}_${subName}`);

          if (!sublocationId) {
            console.warn(`[Sync Warning] Missing sublocation ID for "${locationId}_${subName}"`);
            return;
          }
          ...
          // A. Upsert inventory bin using composite key [inventoryId, sublocationId]
          ...
          // B. Write transaction log if quantity changed
