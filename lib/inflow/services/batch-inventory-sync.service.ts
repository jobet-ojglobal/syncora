...
import { InflowProduct, InflowInventoryLine } from "@/lib/inflow/types";
...
type SyncCache = {
  ...
  sublocationIds: Map<string, string>;
};
...
export class InventorySyncService {
  ...
  /**
   * Group and aggregate inventory lines per location & sublocation combination
   * before applying database changes.
   */
  private aggregateInventoryLines(
    lines: InflowInventoryLine[],
    selectedLocations: Set<string> | null,
    isTrackSerials: boolean
  ): Map<string, AggregatedGroup> {
    const groups = new Map<string, AggregatedGroup>();

    for (const inv of lines) {
      const targetLocationId = inv.locationId || inv.location?.locationId;
      ...
      const groupKey = `${targetLocationId}:${sublocationName}`;
      ...
      // Determine quantity for this line
      let lineQty = Number(inv.quantityOnHand);

      // Fix: If quantity is missing/invalid but serial exists, count as 1
      if ((!Number.isFinite(lineQty) || lineQty === 0) && incomingSerials.length > 0) {
        lineQty = incomingSerials.length;
      } else if (!Number.isFinite(lineQty) || lineQty < 0) {
        lineQty = 0;
      }
      ...
      existingGroup.totalQty += lineQty;
      existingGroup.serials.push(...incomingSerials);
  ...
  async processBatch(
    ...
    for (const product of products) {
      ...
      // 1. Group all lines across locations/sublocations
      const aggregatedGroups = this.aggregateInventoryLines(
        product.inventoryLines,
        selectedLocations,
        isTrackSerials
      );

      // 2. Process each location grouping
      for (const group of aggregatedGroups.values()) {
        const { targetLocationId, sublocationName, totalQty, serials: incomingSerials } = group;

        // Validation: Verify Serial Count === Total StockOnHand for Tracked Items
        if (isTrackSerials && incomingSerials.length !== totalQty) {
          console.error(
            `[Sync Validation Error] Mismatch for product "${product.name}" (${product.productId}) at sublocation "${sublocationName || "FLOOR STOCK"}": ` +
              `Serial count (${incomingSerials.length}) does not match aggregated stock on hand (${totalQty}). Skipping.`
          );
        ...
        try {
          ...
          const quantityDelta = totalQty - currentBinQuantity;
          ...
          await prisma.$transaction(async (tx) => {
            ...
            const inventory = await tx.inventory.upsert({
              ...
              create: {
                ...
                quantityAvailable: Math.max(0, totalQty - currentReserved),
            ...
            // 2. Upsert Sublocation Bin (if sublocation exists)
              ...
              for (const serialNum of incomingSerials) {
                await tx.inventoryBinItem.upsert({
                  ...
                  create: {
                    ...
                    productId: productMeta!.productId,
                    locationId: targetLocationId,
                    inventoryBinId: binId, // null if floor stock / no sublocation
                    status: "IN_STOCK",
                  },
                  update: {
                    locationId: targetLocationId,
  ...
  async batchSync(
    ...
    while (true) {
      ...
      const rawBatch: InflowProduct[] = await getInventoryLevels(BATCH_SIZE, cursor);
