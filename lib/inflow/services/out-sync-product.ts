...
/**
 * Pure helper function to sanitize UOM codes against allowed values.
 */
function sanitizeUom(val: string | null | undefined, allowedSet: Set<string>): string {
  if (!val) return "ea.";
  const clean = val.trim().toLowerCase();
  return clean && allowedSet.has(clean) ? clean : "ea.";
}

/**
 * Maps local database relational products into Inflow API payloads.
 */
export function mapLocalProductToInflowPayload(
  ...
  const customFields: InflowCustomFields = {
    ...((product.customFields as Record<string, string>) || {}),
  };

  const brandName = product.brand?.name;
  if (brandName) {
    const customKey = brandCustomName || "custom7";
    customFields[customKey as keyof InflowCustomFields] = brandName;
  }
  ...
  const mappedVendor: InflowVendor | undefined = product.lastVendor
    ? {
        ...
        discount: product.lastVendor.discount != null ? String(product.lastVendor.discount) : null,
        ...
        lastModifiedById: defaultMember || null,
  ...
  const mappedCost = productCost
    ? {
        cost: productCost.cost != null ? String(productCost.cost) : "0",
        productCostId: productCost.inflowId,
        productId: product.inflowId || "",
      }
    : undefined;

  // 3. Resolve Default Image & Cloudinary Images
  const primaryImage = product.images?.[0];
  const primaryUrl = primaryImage?.originalUrl;

  const mappedDefaultImage =
    primaryImage && primaryUrl?.includes("cloudinary.com")
      ? {
  ...
  // 4. Resolve Purchasing & Sales UOM
  const purchasingUomName = product.purchasingUom?.uom?.name?.trim();
  const mappedPurchasingUom =
    product.purchasingUom && purchasingUomName
      ? {
          name: purchasingUomName,
          conversionRatio: {
            standardQuantity: product.purchasingUom.standardQuantity != null ? String(product.purchasingUom.standardQuantity) : "1",
            uomQuantity: product.purchasingUom.uomQuantity != null ? String(product.purchasingUom.uomQuantity) : "1",
          },
        }
      : undefined;

  const salesUomName = product.salesUom?.uom?.name?.trim();
  const mappedSalesUom =
    product.salesUom && salesUomName
      ? {
          name: salesUomName,
          conversionRatio: {
            standardQuantity: product.salesUom.standardQuantity != null ? String(product.salesUom.standardQuantity) : "1",
            uomQuantity: product.salesUom.uomQuantity != null ? String(product.salesUom.uomQuantity) : "1",
          },
        }
      : undefined;
  ...
  return {
    ...
    standardUomName: product.standardUomName?.trim()
      ? sanitizeUom(product.standardUomName, ALLOWED_UOM_SET)
      : "",
    trackExpiry: false,
    trackLots: false,
    ...
    categoryId: cat?.inflowId || defaultCategory || null,
    lastVendorId: null, // product.lastVendorId || null,
    lastModifiedById: "8ff3e71d-eb02-425d-8e0f-00a69fc8e482", // product.lastModifiedById || defaultMember || null,
    cost: mappedCost,
    defaultImage: mappedDefaultImage,
    purchasingUom: mappedPurchasingUom,
    salesUom: mappedSalesUom,
    customFields,
    images: mappedImages,
    prices: (product.prices || []).map((p) => ({
      ...
      unitPrice: p.unitPrice != null ? String(p.unitPrice) : "0",
      fixedMarkup: p.fixedMarkup != null ? String(p.fixedMarkup) : "0",
    })),
  };
}
...
export class ProductOutSyncService {
  ...
  async processBatchBulk(
    ...
    try {
      const syncedProducts = await upsertProductBulk(payloads);
      ...
      console.warn(
        `[Product Sync] Bulk array payload failed (${bulkError?.message || "Error"}). Falling back to item-by-item processing...`
      );
    ...
    console.log(
      `[Product Sync] Bulk Batch processing finished in ${this.formatDuration(batchDuration)} (Avg: ${this.formatDuration(batchDuration / products.length)}/item)`
    );
  ...
  async sync(options: SyncOptions, brandCustomName?: string, selectedRecords?: string[]) {
    ...
    if (!defaultTeamMember) {
      console.error("[ProductOutSyncService] Sync aborted: Default team member not found.");
      return { productsProcessed: 0, syncedAt: new Date().toISOString() };
    }
