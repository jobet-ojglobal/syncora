...
import { PostAdjustmentPayload } from "@/services/stock-adjustment.service";
  ...
  async sync(
    ...
    while (true) {
      ...
      if (!rawBatch || rawBatch.length === 0) {
        console.log(`[InventoryCloudSyncService] No more unsynced products found. Sync complete.`);
        break;
      }

      console.log(
        `[InventoryCloudSyncService] Fetched ${rawBatch.length} unsynced items in ${this.formatDuration(fetchDuration)}`
      );
